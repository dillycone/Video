...
export default function VideoProcedureContext() {
  ...
  useEffect(() => {
    const loadPrompt = async () => {
      try {
        const promptPath = pdfFile 
          ? '/prompts/procedure_from_video_prompt_with_additional_context.txt'
          : '/prompts/procedure_from_video_prompt.txt';
  ...
  const exportToPDF = () => {
    ...
    yPosition += 10;
    pdf.setFontSize(textSize);
    procedure.prerequisites.forEach((prereq: string) => {
    ...
    yPosition += 10;
    pdf.setFontSize(textSize);
    procedure.troubleshooting.forEach((issue: string) => {
